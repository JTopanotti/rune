...
take_while_check_empty(|c| c.is_ascii_digit(), s, "expected digits".to_string())
}
...
pub(crate) fn extract_whitespace_check_empty(s: &str) -> Result<(&str, &str), String> {
    take_while_check_empty(|c| WHITESPACE.contains(&c), s, "expected a space".to_string())
}
...
fn take_while_check_empty(accept: impl Fn(char) -> bool, s: &str, error_msg: String) -> Result<(&str, &str), String> {
...
mod tests {
    ...
    fn do_not_extract_spaces_when_input_does_not_start_with_them() {
        assert_eq!(
            extract_whitespace_check_empty("blah"),
    ...
    fn extract_alphanumeric_ident() {
        assert_eq!(extract_ident("abcdEFG5 stop"), Ok((" stop", "abcdEFG5")));
    }

    #[test]
    fn extract_alphabetic_ident() {
        assert_eq!(extract_ident("foobar2()"), Ok(("()", "foobar2")));
    }
